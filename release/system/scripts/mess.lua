...
function getBreakfastMenu()
	...
	if file == nil then
		...
		if tonumber(os.date("%H")) >= BEAKFAST_THRESHOLD_HOUR then							-- Check Threshold Time
			...
			breakfast_menu = breakfast_menu .. "                            Menu: "
			if number_of_items > 0 then
				breakfast_menu = breakfast_menu .. breakfast_items[1]
			else
				return "<[W-1]> " .. breakfast_menu .. "Latest Menu has not been received from SSMS."
			end
...
function getLunchMenu()
	...
	if file == nil then
		...
		if tonumber(os.date("%H")) >= LUNCH_THRESHOLD_HOUR then							-- Check Threshold Time
			...
			lunch_menu = lunch_menu .. "                            Menu: "
			if number_of_items > 0 then
				lunch_menu = lunch_menu .. lunch_items[1]
			else
				return "<[W-1]> " .. lunch_menu .. "Latest Menu has not been received from SSMS."
			end
...
function getDinnerMenu()
	...
	if file == nil then
		...
		if tonumber(os.date("%H")) >= DINNER_THRESHOLD_HOUR then							-- Check Threshold Time
			...
			dinner_menu = dinner_menu .. "                            Menu: "
			
		if number_of_items > 0 then
				dinner_menu = dinner_menu .. dinner_items[1]
		else
				return "<[W-1]> " .. dinner_menu .. "Latest Menu has not been received from SSMS."
		end
		-- Concatenate Menu --
		local counter = 2
		while counter <= number_of_items do
